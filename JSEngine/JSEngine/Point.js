...
class Point {
  ...
  speedColor(baseColor = new RGBColor(255, 0, 0), brightnessAmplifier = 1, minBrightness = 0.1){
    let velocity = Math.max(minBrightness, (Math.abs(this.velocity.x) + Math.abs(this.velocity.y)) * brightnessAmplifier);
    baseColor.combine(new RGBColor(velocity, velocity, velocity));
    this.color = baseColor.convertToColor();
  }
  move(wallMargin = 1, wallOffset = 5, minSpeedX = 0, minSpeedY = 0) {
    if (this.pos.x - this.radius <= wallMargin) {
      this.velocity.x = makePositive(this.velocity.x);
      this.pos.x = wallMargin + wallOffset + this.radius;
    }
    if (width - this.radius - this.pos.x <= wallMargin) {
      this.velocity.x = -makePositive(this.velocity.x);
      this.pos.x = width - (wallMargin + wallOffset + this.radius);
    }
    if (this.pos.y - this.radius <= wallMargin) {
      this.velocity.y = makePositive(this.velocity.x);
      this.pos.y = wallMargin + wallOffset + this.radius;
    }
    if (height - this.radius - this.pos.y <= wallMargin) {
      this.velocity.y = -makePositive(this.velocity.x);
      this.pos.y = height - (wallMargin + wallOffset + this.radius);
    }
    if(minSpeedX>0){
      if(Math.abs(this.velocity.x) < minSpeedX) this.velocity.x = minSpeedX/this.velocity.x;
    }
    if(minSpeedY>0){
      if(Math.abs(this.velocity.y) < minSpeedY) this.velocity.y = minSpeedY/this.velocity.y;
    }
